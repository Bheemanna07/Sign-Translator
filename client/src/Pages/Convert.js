...
function Convert() {
  ...
  // ✅ Initialize 3D Avatar and Scene
  useEffect(() => {
    ref.flag = false;
    ref.pending = false;
    ref.animations = [];
  ...
  // ✅ Clean input (fixes speech issues)
  const cleanText = (raw) => {
    if (!raw) return "";
    raw = raw.trim().toLowerCase();
    raw = raw.replace(/[.,!?]/g, ''); // remove punctuation
    return raw.toUpperCase();
  };

  // ✅ Convert text/speech into sign animations
  const sign = (inputRef) => {
    var raw = inputRef.current.value;
    var str = cleanText(raw);
  ...
  // ✅ Optional auto animation trigger when speech stops
  useEffect(() => {
    if (!listening && transcript) {
      // Uncomment this line to start animation automatically after you stop speaking
      // sign(textFromAudio);
    }
  }, [listening]);

  // ✅ Update textarea from transcript
  useEffect(() => {
    if (textFromAudio.current) {
